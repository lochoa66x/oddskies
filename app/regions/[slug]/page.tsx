...
export function generateStaticParams() {
  return regionDefinitions.map((region) => ({ slug: region.slug }));
}
...
export async function generateMetadata({
  ...
  const region = getRegionDefinition(slug);

  if (!region) {
    return {
      title: "Region Not Found | OddSkies",
    };
  }

  const reports = getReportsForRegion(getFieldLogReports(await getReports()), region);
  const description =
    reports.length > 0
      ? getTaxonomyPageDescription("region", region.label, reports.length)
      : `Latest unverified OddSkies field notes from ${region.label}. Source-linked where possible, never confirmed.`;
  const title = `${region.label} Reports | OddSkies`;
...
export default async function RegionPage({ params }: RegionPageProps) {
  ...
  const reports = getReportsForRegion(getFieldLogReports(await getReports()), region);
  const relatedRegions = regionDefinitions.filter(
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mt-8 border-t border-night-800 pt-5">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {relatedRegions.slice(0, 6).map((item) => (
              <Link
