...
} from "../lib/collectors/youtube-core.mjs";
...
const config = createYoutubeCollectorConfig(process.env, {
...
try {
  summary = await collectYoutube(config);
} catch (error) {
...
function parseArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--feed") {
      const feed = args[index + 1];

      if (feed) {
        parsed.feeds.push(feed);
      }
...
function printHelp() {
  console.log(`OddSkies YouTube feed collector prototype

Usage:
  npm run collect:youtube -- --dry-run
  npm run collect:youtube -- --dry-run --feed "Channel name|UCxxxxxxxxxxxxxxxxxxxxxx|UFO / UAP" --limit 5
  npm run collect:youtube -- --feed "Playlist name|PLxxxxxxxxxxxxxxxxxxxxxx|Paranormal" --limit 3
  npm run collect:youtube -- --feed "Feed name|https://www.youtube.com/feeds/videos.xml?channel_id=UC..." --limit 3

Feed config:
  ODDSKIES_YOUTUBE_FEEDS can be JSON or one feed per line:
  Feed name|channel_id_or_playlist_id_or_feed_url|Optional Category
...
Safety:
  YouTube rows go only into public.raw_sources. They are never promoted to
...
async function startCollectorRun(config, mode) {
  ...
  try {
    ...
    const response = await fetch(endpoint, {
      body: JSON.stringify({
        collector_name: "youtube-feeds",
        dry_run: config.dryRun,
        mode,
        platform: "youtube",
        query_count: config.feeds.length,
        status: "started",
      }),
      headers: {
  ...
  try {
    ...
    await fetch(endpoint, {
      body: JSON.stringify({
        ...
        query_count: summary.feeds.length,
...
function inferCollectorRunStatus(summary) {
  if (summary.errors.length > 0 && summary.feeds.length === 0) {
    return "failed";
  }
