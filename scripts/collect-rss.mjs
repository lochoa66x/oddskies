...
} from "../lib/collectors/rss-core.mjs";
...
const config = createRssCollectorConfig(process.env, {
...
try {
  summary = await collectRss(config);
} catch (error) {
...
function printHelp() {
  console.log(`OddSkies RSS collector prototype

Usage:
  npm run collect:rss -- --dry-run
  npm run collect:rss -- --dry-run --feed "Feed name|https://example.com/feed.xml" --limit 5
  npm run collect:rss -- --feed "Feed name|https://example.com/feed.xml|Strange Lights" --limit 3

Feed config:
  ODDSKIES_RSS_FEEDS can be JSON or one feed per line:
  Feed name|https://example.com/feed.xml|Optional Category
...
Safety:
  RSS rows go only into public.raw_sources. They are never promoted to
