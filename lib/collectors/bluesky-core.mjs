  ...
  for (const query of runtimeConfig.queries) {
    ...
    try {
      ...
      for (const post of posts) {
        ...
        const safetyReason =
          typeof runtimeConfig.safetyGate === "function"
            ? runtimeConfig.safetyGate(rawSource)
            : null;

        if (safetyReason) {
          querySummary.exclusionsSkipped += 1;
          continue;
        }
    ...
    summary.totals.exclusionsSkipped += querySummary.exclusionsSkipped;
...
export async function discoverBlueskyCandidates(runtimeConfig) {
  const dateWindow = runtimeConfig.dateWindow ?? {
    errors: [],
    since: null,
    until: null,
  ...
  for (const query of runtimeConfig.queries) {
    ...
    try {
      ...
      for (const post of posts) {
        ...
        const canCheckDuplicates =
          ...
          !runtimeConfig.supabaseServiceRoleKey.startsWith("sb_publishable_");

        if (canCheckDuplicates) {
          duplicateRisk = await rawSourceExists(runtimeConfig, rawSource);
        }

        querySummary.candidates.push({
          duplicateRisk,
          rawSource,
        });
      }
    ...
    summary.totals.fetched += querySummary.fetched;
    summary.totals.repliesSkipped += querySummary.repliesSkipped;
    summary.totals.candidates += querySummary.candidates.length;
    summary.totals.duplicateRisk += querySummary.candidates.filter(
      (candidate) => candidate.duplicateRisk,
    ).length;
    summary.errors.push(...querySummary.errors.map((error) => `${query}: ${error}`));
  }
